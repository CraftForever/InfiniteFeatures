...
public class GuiCustomListWorldSelectionEntry implements GuiListExtended.IGuiListEntry {
	...
	private final GuiCustomWorldSelection customworldSelScreen;
	private final GuiWorldSelection worldSelScreen;
	private final WorldSummary worldSummary;
	private final ResourceLocation iconLocation;
	private final GuiCustomListWorldSelection containingListSel;
	private File iconFile;
	private DynamicTexture icon;
	private long lastClickTime;

	public GuiCustomListWorldSelectionEntry(GuiCustomListWorldSelection guiCustomListWorldSelection,
			WorldSummary worldSummaryIn, ISaveFormat saveFormat) {
		this.containingListSel = guiCustomListWorldSelection;
		this.customworldSelScreen = guiCustomListWorldSelection.getCustomGuiWorldSelection();
		worldSelScreen = guiCustomListWorldSelection.getGuiWorldSelection();
		...
		if (this.worldSummary.requiresConversion()) {
			...
			if (this.worldSummary.markVersionInList()) {
				if (this.worldSummary.askToOpenWorld()) {
					s2 = s2 + ", " + I18n.format("selectWorld.version") + " " + TextFormatting.RED + s3
							+ TextFormatting.RESET;
				} else {
					s2 = s2 + ", " + I18n.format("selectWorld.version") + " " + TextFormatting.ITALIC + s3
							+ TextFormatting.RESET;
				}
			} else {
		...
		if (this.client.gameSettings.touchscreen || isSelected) {
			...
			if (this.worldSummary.markVersionInList()) {
				...
				if (this.worldSummary.askToOpenWorld()) {
					...
					if (j < 32) {
						this.customworldSelScreen.setVersionTooltip(
		...
		if (this.worldSummary.askToOpenWorld()) {
			this.client.displayGuiScreen(new GuiYesNo(new GuiYesNoCallback() {
				...
				public void confirmClicked(boolean result, int id) {
					if (result) {
						GuiCustomListWorldSelectionEntry.this.loadWorld();
					} else {
						GuiCustomListWorldSelectionEntry.this.client
								.displayGuiScreen(GuiCustomListWorldSelectionEntry.this.customworldSelScreen);
					}
				}
	...
	public void deleteWorld() {
		this.client.displayGuiScreen(new GuiYesNo(new GuiYesNoCallback() {
			...
			public void confirmClicked(boolean result, int id) {
				if (result) {
					GuiCustomListWorldSelectionEntry.this.client.displayGuiScreen(new GuiScreenWorking());
					ISaveFormat isaveformat = GuiCustomListWorldSelectionEntry.this.client.getSaveLoader();
					isaveformat.flushCache();
					isaveformat.deleteWorldDirectory(GuiCustomListWorldSelectionEntry.this.worldSummary.getFileName());
					GuiCustomListWorldSelectionEntry.this.containingListSel.refreshList();
				}
	...
	public void editWorld() {
		this.client.displayGuiScreen(new GuiWorldEdit(this.customworldSelScreen, this.worldSummary.getFileName()));
	}

	public void recreateWorld() {
		this.client.displayGuiScreen(new GuiScreenWorking());
		GuiCreateWorld guicreateworld = new GuiCreateWorld(this.customworldSelScreen);
	...
	private void loadWorld() {
		...
		if (this.client.getSaveLoader().canLoadWorld(this.worldSummary.getFileName())) {
			...
			net.minecraftforge.fml.client.FMLClientHandler.instance().tryLoadExistingWorld(worldSelScreen,
					this.worldSummary);
		}
	}
